...
var Compiler = require('./contestCompiler');
var Submission = require('./models/Contest');
...
function Judge(job, fn){
  ...
  logger.debug(chalk.green('Starting contest judge submission id ' + judge.id));


  async.waterfall([
    function(callback){
      submission.find(['s.pid','s.uid','s.cid','s.status','s.language','p.cpu','p.memory'], function(err, rows){
        if(err){
          return callback(err);
        }
        if(!rows.length){
...
//
function makeFinalStatus(runs, submission, judge, fn){
  ...
  async.series([
    ...
    function(callback){
      submission.putRank(judge.cid, judge.uid, judge.pid, status, callback);
    }
  ],
  function(err){
    if(err){
