 ...
 *
 *
 * TO COMPILE: gcc myutil.c -o safejudge dev.c -lm
 *
...
int get_memory_usage() {
    int fd, data, stack,VmSize = 0,VmRSS = 0, VmPeak = 0;
    ...
    sscanf(vm, "%*s %d", &VmSize);
    
    vm = strstr(buf, "VmRSS:");
    sscanf(vm, "%*s %d", &VmRSS);
    
    vm = strstr(buf, "VmPeak:");
    sscanf(vm, "%*s %d", &VmPeak);
    
    printf("VmData = %d, VmStk = %d, VmRSS = %d, VmSize = %d, VmPeak = %d\n",data, stack, VmRSS, VmSize, KBtoMB(VmPeak));
   printf("\n");
...
int getHeapMemory(char * s){

	char * pch;
	int highMemmory = -1, lowmemory = -1;
	
	pch = strtok (s," ");
	if (pch != NULL){
		pch = strtok (pch,"-");
		if( pch != NULL ){
			//printf("\n heap is %s = %d && ", pch, hexadecimal_to_decimal(pch));
			lowmemory = hexadecimal_to_decimal(pch);
			
			
			pch = strtok (NULL, "-");
			if( pch != NULL ){
				//printf("%s = %d", pch, hexadecimal_to_decimal(pch));
				highMemmory = hexadecimal_to_decimal(pch);
			}
		}
	}
...
void get_smaps() {
    int fd;
    char buf[4096], buf2[4096], status_child[30];

    sprintf(status_child, "/proc/%d/smaps", pid);
    if ((fd = open(status_child, O_RDONLY)) < 0)
        return;
...
void get_statm() {
    ...
    sprintf(status_child, "/proc/%d/statm", pid);
    if ((fd = open(status_child, O_RDONLY)) < 0)
        return;
	...
	pch = strtok (buf," ");
	  ...
	  int page = (int)(getpagesize() / 1024);
		
    printf(" actual = %dMB, page = %d\n", KBtoMB(actual * page), page);
}
...
void handleChild(){
    ...
    execl("/usr/bin/java", "java", "-Xmx1024m", "-Xms128m", "-cp", RUN_DIR,programFile, NULL);
...
#ifdef DEBUG
			//fprintf(stderr ,"getpagesize memory ==  %ld, ", (getpagesize() * rused.ru_minflt)/1024 );
			fprintf(stderr ,"ru_maxrss memory ==  %dMB, ", KBtoMB((int)(rused.ru_maxrss)) );
#endif
...
#ifdef DEBUG
        fprintf(stderr ,  "memoryUsed = %dMB , limit = %dMB, " , KBtoMB(memoryUsed), memoryLimit);
       // get_statm();
       get_smaps();
#endif
