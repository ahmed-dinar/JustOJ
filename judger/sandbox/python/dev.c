...
int get_memory_usage() {
    ...
    printf("VmData = %d, VmStk = %d, VmRSS = %d, VmSize = %d\n",data, stack, VmRSS, VmSize);
...
#ifdef DEBUG
    fprintf(stderr,"Executing file %s, under user id: %d , errno = %s, RUN_DIR = %s\n",FielPRog,getuid(),strerror(errno), RUN_DIR);
#endif


    //execl(FielPRog, FielPRog, NULL);
   // char *const params[] = {"/usr/bin/python2", FielPRog, NULL};
    
    
#ifdef DEBUG
   // fprintf(stderr,"param[0] %s\n",params[0]);
    fprintf(stderr,"programFile =  %s\n",programFile);
#endif
    
    execl("/usr/local/bin/python3.5", "python3.5", FielPRog, NULL);
    //execv(params[0], params);
   // execl("/usr/bin/java","java", "-Xmx1024m", "-Xss1024m", "Main", NULL);
...
void handleParent(){
    ...
    while(1){
        ...
        if (WIFSIGNALED(status)){
			...
			fprintf(stderr ,"ru_maxrss memory ==  %ld, ", (rused.ru_maxrss) );
#endif
...
#ifdef DEBUG
        fprintf(stderr ,  "memoryUsed = %d , limit = %d, " ,memoryUsed,MBtoKB(memoryLimit));
        get_statm();
#endif
